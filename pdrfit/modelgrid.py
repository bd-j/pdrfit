...
class ModelLibrary(object):
    ...
    def add_par(self, value, name, dtype='<f8'):
        arrs = [self.pars, self.structure_array(value, [name],
                                                types=[dtype])]
        self.pars = self.join_struct_arrays(arrs)
        pass
    ...
    def par_range(self, parname, inds=None):
        range_list = [(np.nanmin(self.pars[inds][p]),np.nanmax(self.pars[inds][p]), p)
                      for p in parname]
        return range_list
...
class SpecLibrary(ModelLibrary):
    ...
    def read_model_from_fitsbinary(self, filename, parnames,
        ...
        #    raise IOError('read_model_from_fitsbinary: ',filename,' does not exist')
